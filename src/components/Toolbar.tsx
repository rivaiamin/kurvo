    ...
    <div className="flex flex-wrap items-center justify-between p-4 bg-white shadow-sm border-b z-10 relative">
        <div className="flex items-center gap-4">
            <div className={`flex bg-slate-100 p-1 rounded-md border ${isAnimating ? 'opacity-50 pointer-events-none' : ''}`}>
                <button onClick={() => switchTool('draw')} className={`px-3 py-1.5 rounded text-sm font-medium flex items-center gap-2 transition-all ${activeTool === 'draw' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>
                    <PenTool size={16} /> Draw
                </button>
                <button onClick={() => switchTool('select')} className={`px-3 py-1.5 rounded text-sm font-medium flex items-center gap-2 transition-all ${activeTool === 'select' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>
                    <MousePointer2 size={16} /> Select Box
                </button>
                <button onClick={() => switchTool('edit')} className={`px-3 py-1.5 rounded text-sm font-medium flex items-center gap-2 transition-all ${activeTool === 'edit' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>
                    <Move size={16} /> Edit Nodes
                </button>
                <button onClick={() => switchTool('pressure')} className={`px-3 py-1.5 rounded text-sm font-medium flex items-center gap-2 transition-all ${activeTool === 'pressure' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'}`}>
        ...
        <div className="flex flex-wrap items-center gap-2 mt-2 md:mt-0">
            ...
            <button onClick={() => { if (projectRef.current?.activeLayer) { projectRef.current.activeLayer.removeChildren(); switchTool('draw'); } }} className={`p-2 text-red-500 hover:bg-red-50 rounded-md ${isAnimating ? 'opacity-50 pointer-events-none' : ''}`}>
                <Trash2 size={20} />
            </button>
